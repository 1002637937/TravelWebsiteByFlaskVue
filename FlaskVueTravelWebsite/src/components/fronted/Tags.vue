<template lang="html">
  <div class="tags_wrap"  v-loading="loading2"  element-loading-text="加载中">
    ...
    <div class='classify_wrapper'>
      <Classify v-for="class_ in classes_" :name="class_.classify" page='tags' :id='class_.id' :img_path='class_.img_path'/>
    </div>
    <!-- </ul> -->
...
export default {
  ...
  mounted(){
    this.$store.dispatch('changeHeadLine','景区')
      // 根据景区名获取游记列表
    let place_id = this.$route.query.place_id
    if (place_id) {
      api.getClassifyByPlace({"id":place_id})
        .then((res) => {
          if(res.status == 200){
            this.loading2=false;
